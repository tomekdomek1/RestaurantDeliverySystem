...
export default function AdminMenuPage() {
  ...
  useEffect(() => {
    fetch('/api/dishes')
      ...
      .catch(() => console.log('Upewnij się, że json-server działa na porcie 3000'));
  }, []);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box>
          <Typography variant="h4" fontWeight="800" color="#0b1437">Zarządzanie Menu</Typography>
          <Typography variant="body2" color="text.secondary">Zarządzaj ofertą swojej restauracji w czasie rzeczywistym.</Typography>
        </Box>
        <Button variant="contained" size="large" sx={{ bgcolor: '#0b1437', borderRadius: '12px', px: 4 }}>+ Nowe Danie</Button>
      </Box>
        ...
        <TableBody>
          {dishes.map((dish) => (
            <TableRow key={dish.id} sx={{ '&:hover': { bgcolor: '#fafafa' } }}>
              ...
              <TableCell>
                <Chip label={`${dish.price.toFixed(2)} zł`} sx={{ fontWeight: 'bold', bgcolor: '#e8f5e9', color: '#2e7d32' }} />
              </TableCell>
